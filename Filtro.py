...
def search_csv(files, folder):
    # Diccionario para almacenar qué debe cada persona junto con su fecha de entrada, correo y job function
    pendientes = defaultdict(lambda: {'cursos': [], 'fecha': '', 'email': '', 'job_function': ''})
    ...
    with open(os.path.join(folder, 'resultados.csv'), 'w', newline='') as results_file:
        ...
        writer.writerow(['Empleado', 'Email', 'Fecha de Entrada', 'Job Function', 'Cursos pendientes'])
        
        # Escribir los resultados
        for empleado, info in pendientes.items():
            writer.writerow([empleado, info['email'], info['fecha'], info['job_function'], ', '.join(info['cursos'])])
